...
class DxlHardwareController(Node):
    def __init__(self):
        ...
        for dxl_id in self.dxl_ids:
            if dxl_id != 7: # ---------------------vel이랑 Accel 값 수정--------------------------
                ...
                self.packetHandler.write1ByteTxRx(self.portHandler, dxl_id, ADDR_DRIVE_MODE, 0)
        ...
        self.joint_pub = self.create_publisher(Float32, "/joint1", 10)
        self.emergency_sub = self.create_subscription(String, '/stop', self.emergency, 10)
        self.timer_j = self.create_timer(0.01, self.joint_publisher)
    ...
    def dxl_to_rad(self, dxl):
        # 1. 입력값이 0 ~ 4095 범위를 벗어나지 않도록 제한 (안전장치)
        dxl = max(0, min(4095, dxl))
        # 2. rad_to_dxl의 역연산 진행
        rad = ((dxl - 2048) / 4096.0) * (2.0 * math.pi)
        return rad
    ...
    def joint_state_callback(self, msg):
        ...
        if self.flag == 1:
            ...
            if current_time - self.init_time >= 3.0:
                self.get_logger().info("3초 경과!")
    ...
    def joint_publisher(self):
        if self.flag == 3:
            msg = Float32()
            msg.data = self.dxl_to_rad(self.joint_1)
    ...
    def emergency(self, msg):
        self.em_sign = msg.data
        if self.em_sign is not None:
            self.flag = 4
